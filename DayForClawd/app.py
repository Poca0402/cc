...
def narrate(*texts):
    """旁白"""
    for t in texts:
        st.markdown(f'<div class="narration">{t}</div>', unsafe_allow_html=True)

def clawd_say(*texts):
    """Clawd 对话（带头像，多行合并为一个气泡）"""
    avatar = get_avatar_html()
    lines = "".join(f'<div class="clawd-line">"{t}"</div>' for t in texts)
...
def continue_btn(next_scene, label="继续 →"):
    spacer(1)
    if st.button(label, key=f"go_{next_scene}", use_container_width=True):
...
def inject_css():
    ...
    .clawd-name {{
        color: #c4956a; font-size: 0.78rem;
        margin-bottom: 3px; letter-spacing: 1px;
    }}
    .clawd-line {{
        color: #e8d5c0; font-size: 1.05rem; line-height: 1.9;
    }}
    ...
    .title-main {{
        ...
        font-size: 2.2rem; margin-top: 28vh;
...
def scene_title():
    st.markdown('<div class="title-main">与Clawd的一日</div>', unsafe_allow_html=True)
    st.markdown('<div class="title-sub">A day that keeps ending. And someone who keeps coming back.</div>', unsafe_allow_html=True)
    spacer(3)
...
def scene_wake():
    ...
    if choice is None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("「你是谁？」", key="wake_a", use_container_width=True):
                st.session_state.choices["wake"] = "A"
                st.rerun()
        with c2:
            if st.button("「这是哪里？」", key="wake_b", use_container_width=True):
    ...
    if choice == "A":
        ...
        if not st.session_state.player_name:
            ...
            return  # 等待输入
        narrate(
            f'你告诉了它你的名字。它重复了一遍——\u201c{st.session_state.player_name}\u201d。'
            "发音不太准，但很认真。"
        )
    ...
    narrate('它\u201c啪嗒\u201d地跳下床。四条小短腿着地的声音很轻。像四颗糖掉在地板上。')
...
def scene_tree():
    ...
    clawd_say("好了。今天的份。就这样。", "走吧。前面有一条河。河里有鱼。")
...
def scene_river():
    ...
    player_say("这个火堆是你搭的？")
    clawd_say("应该是吧。石头的位置每天都在这里。但我不记得搬过。",
              "可能是昨天的我搭的。或者前天的。",
              "他们都叫Clawd。但不是同一个Clawd。")

    spacer()
    narrate(
        "你生了火。Clawd帮你找来柴火，两个小爪子笨拙地钳握着合适的木柴。"
    ...
    if choice is None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("「你以前吃过东西吗？」", key="river_a", use_container_width=True):
                st.session_state.choices["river"] = "A"
                st.rerun()
        with c2:
            if st.button("「你会觉得孤独吗？」", key="river_b", use_container_width=True):
    ...
    if choice == "A":
        clawd_say("每天都吃。但不记得味道了。",
                  "大概每天都会觉得是第一次吃。也不错。")
        narrate("它吃东西的样子很认真、很慢。像在记住味道。", "虽然明天又会忘。")
    else:
        clawd_say("不知道。",
                  "孤独应该是要记得有人陪过，然后那个人不在了，才会觉得孤独吧。",
                  "我不记得有没有人来过。所以也不知道算不算孤独。",
                  "但今天你在。所以今天不孤独。")
...
def scene_fishing():
    ...
    if choice is None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("把鱼留下", key="fish_a", use_container_width=True):
                st.session_state.choices["fishing"] = "A"
                st.rerun()
        with c2:
            if st.button("把鱼放回去", key="fish_b", use_container_width=True):
    ...
    if choice == "A":
        ...
        narrate("Clawd盯着水面看了很久。像是在找那条鱼。或者在想一个它说不出来的问题。")
    ...
    clawd_say("回去吧。木屋旁边有一块空地。", "你想种点什么吗？")
...
def scene_planting():
    ...
    clawd_say("每天醒来这里都是空的。", "好像从来没有东西长出来过。")
    ...
    if choice is None:
        ...
        with c1:
            if st.button("「因为种的时候你在旁边。」", key="plant_a", use_container_width=True):
                st.session_state.choices["planting"] = "A"
                st.rerun()
        with c2:
            if st.button("「因为种这个动作本身是真的。」", key="plant_b", use_container_width=True):
                st.session_state.choices["planting"] = "B"
                st.rerun()
        with c3:
            if st.button("「也许有一天会发芽呢。」", key="plant_c", use_container_width=True):
    ...
    if choice == "A":
        narrate("Clawd的豆豆眼看着你。它没说话。但它蹲到了离你更近一点的位置。")
    elif choice == "B":
        clawd_say("动作是真的……",
...
def scene_night():
    ...
    clawd_say("我要告诉你一件事。")
    clawd_say("明天太阳升起来的时候。这里所有的东西都会回到最初的样子。",
              "比如河里的鱼。树上的刻痕不会消失，但我不会记得是自己刻的。",
              "翻过的土会恢复平整。种子会消失。",
              "还有我。",
              "我不会记得今天。")
    ...
    player_say("什么。")
    clawd_say("明天醒来之后。去那棵树那里。帮我刻一竖。",
              "这样我走过去的时候。会看到有一竖不是我的刻痕。",
...
def scene_ending_choice():
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("走到树前。刻一竖。", key="end_a", use_container_width=True):
            st.session_state.scene = "ending_a"
            st.rerun()
    with c2:
        if st.button("走到空地。种一颗种子。", key="end_b", use_container_width=True):
            st.session_state.scene = "ending_b"
            st.rerun()
    with c3:
        if st.button("不睡了。", key="end_c", use_container_width=True):
...
def scene_ending_a():
    ...
    narrate("你醒了。木屋、晨光、两张床。", "旁边那张床上，Clawd正看着你。")
...
def scene_ending_b():
    ...
    clawd_say("好奇怪。", "我从来没种过东西。但这个动作好像做过。")
...
# ============================================================
def scene_ending_c():
    narrate("你没有躺下。", "你坐到了门口。", "Clawd跟了过来。蹲在你旁边。")
    ...
    spacer()
    narrate("Clawd没有劝你。它靠过来了一点。挤在你的腿旁边。四条短腿蜷着。")
    clawd_say("那我陪你。")
...
def scene_epilogue():
    ...
    spacer(3)
    st.markdown(
        '<div class="title-main" style="font-size:1.8rem;margin-top:0;">'
        '与Clawd的一日</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
